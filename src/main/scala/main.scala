...
case class CLIConf(arguments: Seq[String]) extends ScallopConf(arguments) {
	...
	val writeToCSVResults = opt[String]("csv-file-results", descr = "File name to store pairwise results in booleans in csv format")
	val writeToCSVStats = opt[String]("csv-file-stats", descr = "File name to store general stats in CSV format")
	val writeToCSVPercentage = opt[String]("csv-file-percentage", descr = "File name to store pairwise results in percentages in csv format")
	...
	val threshold = opt[Int]("threshold", descr = "Threshold (0-100) to flag plagiarism", default = Some(30))
	val thresholdMargin = opt[Int]("threshold-margin", descr = "Threshold Margin (0-5) to subtract of threshold",
		default = Some(0))
	...
	def main(args: Array[String]) {
		...
		val computeScores: List[(File, ParseTreeNode)] ⇒ List[((String, String), Double)] = {
			case List((a, parseTreeNodeA), (b, parseTreeNodeB)) ⇒ {
				...
				val aname: String = a.getName// + parseTreeNodeA.start + "-" + parseTreeNodeA.end
				val bname: String = b.getName// + parseTreeNodeB.start + "-" + parseTreeNodeB.end
		...
		val (writeToFileResults, csvResultsFileName) = conf.writeToCSVResults.toOption match {
			case Some(fname) => (true, fname)
			case None => (false, "")
		}
		val (writeToFileStats, csvFileNameStats) = conf.writeToCSVStats.toOption match {
			case Some(fname) => (true, fname)
			case None => (false, "")
		}

		val (writeToFilePercentage, csvFileNamePercentage) = conf.writeToCSVPercentage.toOption match {
			case Some(fname) => (true, fname)
			case None => (false, "")
		}

		val csvFile = if (writeToFileResults) {
			Some(new FileWriter(csvResultsFileName))
		} else {
			None
		}
		val csvFileStats = if (writeToFileStats) {
			Some(new FileWriter(csvFileNameStats))
		} else {
			None
		}
		val csvFilePercentage = if (writeToFilePercentage) {
			Some(new FileWriter(csvFileNamePercentage))
		} else {
			None
		}

		def printAndWrite(s: String, newline: Boolean = true, filt: Boolean = false, stats: Boolean = false, percentage: Boolean = false) {
		...
		printAndWrite("," + names.mkString(","), true, true)
		printAndWrite("," + names.mkString(","), true, false, false, true)

		var amountFalse = 0
		var amountTrue = 0
		var filesNotChecked = 0

		val thresholdMarginFactor: Double = conf.thresholdMargin() / 100.0
		val thresholdFactor: Double = (conf.threshold() / 100.0) - thresholdMarginFactor

		for (name <- names) {
			printAndWrite(name + ",", false, true)
			printAndWrite(name + ",", false, false, false, true)
			for (name2 <- names) {
				val res = if (name == name2) {
					1.0
				} else {
					memo.getOrElse((name, name2), -1.0)
				}
				if ((name != name2) && (res >= 0)) {
					printAndWrite(res + ",", false, false, false, true)
				} else {
					printAndWrite("skip" + ",", false, false, false, true)
				}

				if ((res >= thresholdFactor) && (name != name2)) {
					printAndWrite("True" + ",", false, true)
					amountTrue += 1
				} else if ((0 <= res) && (res < thresholdFactor) && (name != name2)) {
					printAndWrite("False" + ",", false, true)
					amountFalse += 1
				} else {
					printAndWrite("Skip" + ",", false, true)
					filesNotChecked += 1
				}
			}
			printAndWrite("", true, true)
			printAndWrite("", true, false, false, true)
		}
		printAndWrite("No plagiarism" + "," + amountFalse, true, false, true)
		printAndWrite("Possible Plagiarism" + "," + amountTrue, true, false, true)
		printAndWrite("Files not checked" + "," + filesNotChecked, true, false, true)
